...
function s.tfil1(c,e,tp)
	return c:IsLevel(1) and c:IsRace(RACE_PLANT)
		and ((Duel.GetLocCount(tp,"M")>0 and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_DEFENSE))
			or (Duel.GetLocCount(1-tp,"M")>0 and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_DEFENSE,1-tp)))
end
...
function s.op1(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 then
		local tc=g:GetFirst()
		local b1=Duel.GetLocCount(tp,"M")>0 and tc:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_DEFENSE)
		local b2=Duel.GetLocCount(1-tp,"M")>0 and tc:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_DEFENSE,1-tp)
...
function s.tfil22(c,e,tp)
	return c:IsCanBeEffectTarget(e) and c:IsFaceup() and c:IsControlerCanBeChanged()
...
function s.tar2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local ct=#g
	local sg=Duel.GMGroup(s.tfil22,tp,0,"M",nil,e,tp)
	if chk==0 then
	...
	Duel.SOI(0,CATEGORY_CONTROL,tg,#tg,0,0)
end
function s.op2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local g=Duel.GetTargetCards(e):Filter(Card.IsControlerCanBeChanged,nil)
