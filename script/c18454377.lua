...
function s.cfil1(c)
	return c:IsAbleToRemoveAsCost() and (c:IsRace(RACE_FAIRY) or c:IsAttribute(ATTRIBUTE_FIRE))
end
...
function s.tar2(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SPOI(0,CATEGORY_TODECK,nil,1,PLAYER_ALL,"OGR")
end
...
function s.ocon21(e,tp,eg,ep,ev,re,r,rp)
	...
	return s.phase_check(phase) and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(Card.IsAbleToDeck),
		tp,LSTN("OGR"),LSTN("OGR"),1,nil)
end
function s.oop21(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_CARD,0,id)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(Card.IsAbleToDeck),
