...
function s.tg1filter(c)
	return c:IsSetCard(0xf2f) and not c:IsCode(id) and c:IsAbleToHand()
end
...
function s.op1filter(c)
	return c:IsAbleToDeck() and c:IsContinuousSpell()
end
...
function s.op1(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 then
		...
		Duel.ShuffleDeck(tp)
		local dg=Duel.GetMatchingGroup(s.op1filter,tp,LOCATION_GRAVE,0,nil)
...
function s.tg2filter(c,e)
	return (c:IsSetCard(0xf2f) or c:IsContinuousSpell()) and c:IsAbleToDeck() and c:IsCanBeEffectTarget(e)
end
...
function s.tg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return #g>2 and Duel.IsPlayerCanDraw(tp) end
