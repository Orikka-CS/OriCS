...
function s.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(99000098,0))
	...
	e2:SetTarget(function(e,tp,eg,ep,ev,re,r,rp,chk)
		if chk==0 then
			return Duel.IsExistingMatchingCard(aux.TRUE,tp,LOCATION_SZONE,0,1,nil)
		end
	end)
	e2:SetOperation(s.MST_op)
	e2:SetLabel(1)
	c:RegisterEffect(e2)
	--상대의 필드
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(99000098,1))
	...
	e3:SetOperation(s.MST_op)
	e3:SetLabel(2)
	c:RegisterEffect(e3)
	--서로의 필드
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringid(99000098,2))
	...
	e4:SetTarget(function(e,tp,eg,ep,ev,re,r,rp,chk)
		if chk==0 then
			return Duel.IsExistingMatchingCard(aux.TRUE,tp,LOCATION_SZONE,LOCATION_SZONE,1,nil)
		end
	end)
	e4:SetOperation(s.MST_op)
	e4:SetLabel(3)
	c:RegisterEffect(e4)
end
...
function s.MST_op(e,tp,eg,ep,ev,re,r,rp)
	local g=nil
	if e:GetLabel()==1 then
		g=Duel.GetMatchingGroup(aux.TRUE,tp,LOCATION_SZONE,0,nil)
	elseif e:GetLabel()==2 then
		g=Duel.GetMatchingGroup(aux.TRUE,tp,0,LOCATION_SZONE,nil)
	elseif e:GetLabel()==3 then
		g=Duel.GetMatchingGroup(aux.TRUE,tp,LOCATION_SZONE,LOCATION_SZONE,nil)
	end
