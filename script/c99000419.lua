...
function s.initial_effect(c)
	...
	--드로우 페이즈 이외로 자신 / 상대의 카드가 덱에서 벗어났을 경우에 발동할 수 있다. 이 카드를 패에서 특수 소환한다.
	...
	e0:SetCondition(function(_,_,eg) return eg:IsExists(Card.IsPreviousLocation,1,nil,LOCATION_DECK) and not Duel.IsPhase(PHASE_DRAW) end)
	e0:SetTarget(s.target)
	e0:SetOperation(s.operation)
	c:RegisterEffect(e0)
	--덱에서 "페넘브라 베로니카" 이외의 "페넘브라" 몬스터 1장을 수비 표시로 특수 소환한다.
...
function s.spfilter(c,e,tp)
	return c:IsSetCard(0xc11) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_DEFENSE)
end
