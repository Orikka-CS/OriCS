...
function s.initial_effect(c)
	...
	e2:SetCategory(CATEGORY_SEARCH+CATEGORY_RELEASE+CATEGORY_REMOVE)
	e2:SetCL(1,{id,1})
	WriteEff(e2,2,"TO")
	c:RegisterEffect(e2)
	local e3=e2:Clone()
...
function s.tfil2(c,e)
	return c:IsSetCard(0xa4) and c:IsReleasableByEffect(e) and c:IsType(TYPE_MONSTER)
end
function s.tar2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Duel.IEMCard(s.tfil2,tp,"D",0,1,nil,e)
	end
	Duel.SOI(0,CATEGORY_RELEASE,nil,1,tp,"D")
	Duel.SPOI(0,CATEGORY_REMOVE,nil,1,tp,"G")
	Duel.SPOI(0,CATEGORY_TOHAND,nil,1,tp,"D")
end
function s.ofil21(c,tp)
	return c:IsSetCard(0xa4) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemove()
		and Duel.IEMCard(s.ofil22,tp,"D",0,1,nil,c)
end
function s.ofil22(c,rc)
	return c:IsType(TYPE_QUICKPLAY) and c:IsAbleToHand() and rc:ListsCode(c:GetCode())
end
function s.op2(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
	local g=Duel.SMCard(tp,s.tfil2,tp,"D",0,1,1,nil,e)
	local tc=g:GetFirst()
	if tc and Duel.SendtoGrave(g,REASON_EFFECT+REASON_RELEASE)>0 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
		local sg=Duel.SMCard(tp,s.ofil21,tp,"G",0,0,1,nil,tp)
		local sc=sg:GetFirst()
		if sc and Duel.Remove(sg,POS_FACEUP,REASON_EFFECT)>0 then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
			local tg=Duel.SMCard(tp,s.ofil22,tp,"D",0,1,1,nil,sc)
			if #tg>0 then
...
function s.tfil4(c)
	return c:IsCode(15471265) and c:IsAbleToHand()
end
...
function s.oop72(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:GetFlagEffect(id)==0 then
		if rp~=tp and Duel.GetCurrentChain(true)==0 then
