...
function s.tg2filter(c)
	return c:IsSetCard(0xf31) and c:IsAbleToDeck()
end
...
function s.op2(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 then
		...
		if #sg>2 and Duel.IsPlayerCanDraw(tp) and Duel.SelectYesNo(tp,aux.Stringid(id,0)) then
			Duel.BreakEffect()
			Duel.ShuffleDeck(tp)
			Duel.Draw(tp,#sg//3,REASON_EFFECT)
		end
	end
end