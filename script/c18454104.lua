	...
	if chk==0 then
		return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
			and Duel.IsPlayerCanSpecialSummonMonster(tp,id,0xc02,
				TYPE_MONSTER|TYPE_NORMAL,200,3800,8,RACE_ILLUSION,ATTRIBUTE_WIND)
	end
...
function s.op2(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e)
		and Duel.IsPlayerCanSpecialSummonMonster(tp,id,0xc02,
			TYPE_MONSTER|TYPE_NORMAL,200,3800,8,RACE_ILLUSION,ATTRIBUTE_WIND) then
...
function s.tfil3(c)
	return c:IsSetCard(0xc02) and c:IsAbleToGrave() and c:IsType(TYPE_MONSTER)
end
