...
function s.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_REMOVE+CATEGORY_CONTROL+CATEGORY_DRAW)
	WriteEff(e1,1,"TO")
	c:RegisterEffect(e1)
end
...
function s.op1(e,tp,ep,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) then
		...
		if chk>0 then
			local g=Duel.GMGroup(s.ofil1,tp,0,"M",nil)
			if #g>0 and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then
				...
				if Duel.Control(g,tp,PHASE_END,1) then
					local sc=g:GetFirst()
					while sc do
