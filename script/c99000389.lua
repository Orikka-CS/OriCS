...
function s.initial_effect(c)
	--Order summon
	aux.AddOrderProcedure(c,"R",nil,aux.FilterBoolFunction(Card.IsAttribute,ATTRIBUTE_FIRE),s.ordfil1,aux.FilterBoolFunction(Card.IsAttribute,ATTRIBUTE_FIRE))
	c:EnableReviveLimit()
	--이 카드는 오더 소환 및 이하의 방법으로만 특수 소환할 수 있다.
	...
	c:RegisterEffect(e0)
	--자신 메인 페이즈 2 에, 엑스트라 몬스터 존의 자신의 화염 속성 몬스터 1장을 릴리스했을 경우에 엑스트라 덱에서 특수 소환할 수 있다.
	...
	c:RegisterEffect(e3)
	--덱에서 통상 소환 가능한 화염 속성 몬스터 1장을 패에 넣는다.
...
function s.hspfilter(c,tp,sc)
	return c:IsAttribute(ATTRIBUTE_FIRE) and c:IsLocation(LOCATION_MZONE) and c:IsInExtraMZone(tp)
...
function s.descon(e,tp,eg,ep,ev,re,r,rp)
	...
	return bc and bc:IsAttributeExcept(ATTRIBUTE_FIRE) and c:IsSummonType(SUMMON_TYPE_ORDER)
end
...
function s.thfilter(c)
	return c:IsSummonableCard() and c:IsAttribute(ATTRIBUTE_FIRE) and c:IsAbleToHand()
end
