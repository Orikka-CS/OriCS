...
function s.initial_effect(c)
	...
	--자신 묘지에서, 레벨 4 이하의 천사족 / 빛 속성 몬스터 1장을 특수 소환하거나, 카운터 함정 카드 1장을 자신 필드에 세트한다.
	...
	e2a:SetTarget(s.spsettarget)
	e2a:SetOperation(s.spsetop)
...
function s.spsetfilter(c,e,tp,ft)
	return (ft>0 and c:IsLevelBelow(4) and c:IsRace(RACE_FAIRY) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsCanBeSpecialSummoned(e,0,tp,false,false))
		or (c:IsCounterTrap() and c:IsSSetable())
end
function s.spsettarget(e,tp,eg,ep,ev,re,r,rp,chk)
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	if chk==0 then return Duel.IsExistingMatchingCard(s.spsetfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp,ft) end
...
function s.spsetop(e,tp,eg,ep,ev,re,r,rp)
	...
	local sc=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.spsetfilter),tp,LOCATION_GRAVE,0,1,1,nil,e,tp,ft):GetFirst()
	if not sc then return end
	if sc:IsMonster() then
		Duel.SpecialSummon(sc,0,tp,tp,false,false,POS_FACEUP)
	elseif sc:IsTrap() and Duel.SSet(tp,sc)>0 then
		--이 효과로 세트한 카드는 세트한 턴에도 발동할 수 있다.
