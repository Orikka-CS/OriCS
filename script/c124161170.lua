...
function s.cst1(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsReleasable() and Duel.CheckLPCost(tp,500) end
	Duel.PayLPCost(tp,500)
	Duel.Release(e:GetHandler(),REASON_COST)
end

function s.tg1filter(c,e,tp)
	return c:IsSetCard(0xf2b) and c:IsMonster() and not c:IsCode(id) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
