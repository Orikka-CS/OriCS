...
function s.con1(e,tp,eg,ep,ev,re,r,rp)
	return tp~=ep and Duel.GetCurrentChain()==0 and eg:IsExists(s.nfil1,1,nil)
end
...
function s.tfil1(c)
	return c:IsPreviousLocation(LOCATION_EXTRA) and c:IsAbleToExtra()
end
function s.tar1(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return true
	end
	local g=eg:Filter(s.tfil1,nil)
