...
function s.cfil1(c)
	return c:IsAbleToRemoveAsCost() and (c:IsRace(RACE_FAIRY) or c:IsAttribute(ATTRIBUTE_WATER))
end
...
function s.ocon21(e,tp,eg,ep,ev,re,r,rp)
	...
	return s.phase_check(phase) and Duel.IEMCard(s.onfil21,tp,"D",0,1,nil)
end
...
function s.tfil3(c,e,tp)
	return (c:IsRace(RACE_FAIRY) and c:IsAttribute(ATTRIBUTE_WATER)) and c:IsAbleToHand()
end
