...
function s.onfil21(c,e,tp)
	return c:IsSetCard("라일락") and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.ocon21(e)
	...
	local ae=Duel.GetChainInfo(cc,CHAININFO_TRIGGERING_EFFECT)
	local b1=Duel.GetLocCount(tp,"M")>0 and Duel.IEMCard(s.onfil21,tp,"D",0,1,nil,ae,tp)
	local b2=ae:GetHandler():IsRelateToEffect(ae)
	return Duel.GetFlagEffect(tp,id)==0 and (b1 or b2)
end
function s.oop21(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local b1=Duel.GetLocCount(tp,"M")>0 and Duel.IEMCard(s.onfil21,tp,"D",0,1,nil,e,tp)
