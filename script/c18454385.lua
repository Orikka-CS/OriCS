...
function s.cfil1(c)
	return c:IsAbleToRemoveAsCost() and (c:IsRace(RACE_FAIRY) or c:IsAttribute(ATTRIBUTE_EARTH))
end
...
function s.tar2(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SPOI(0,CATEGORY_SPECIAL_SUMMON,nil,1,PLAYER_ALL,"GR")
end
...
function s.ocon21(e,tp,eg,ep,ev,re,r,rp)
	...
	return s.phase_check(phase) and Duel.IEMCard(aux.NecroValleyFilter(Card.IsCanBeSpecialSummoned),
		tp,"GR","GR",1,nil,e,tp)
end
function s.oop21(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.Hint(HINT_CARD,0,id)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SMCard(tp,aux.NecroValleyFilter(Card.IsCanBeSpecialSummoned),tp,"GR","GR",1,1,nil,e,tp)
	if #g>0 then
...
function s.oocon212(e,tp,eg,ep,ev,re,r,rp)
	local rc=re:GetHandler()
	return rc:IsOriginalCodeRule(e:GetLabel()) and rp~=tp
end
function s.ooop212(e,tp,eg,ep,ev,re,r,rp)
