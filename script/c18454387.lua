...
function s.cfil1(c)
	return c:IsAbleToRemoveAsCost() and (c:IsRace(RACE_FAIRY) or c:IsAttribute(ATTRIBUTE_DARK))
end
...
function s.ocon21(e,tp,eg,ep,ev,re,r,rp)
	...
	return s.phase_check(phase) and Duel.IEMCard(Card.IsFaceup,tp,"OGR","OGR",1,nil)
end
function s.oop21(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	Duel.Hint(HINT_CARD,0,id)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	local g=Duel.SMCard(tp,Card.IsFaceup,tp,"OGR","OGR",1,1,nil)
	local tc=g:GetFirst()
	if tc then
		...
		e1:SetTR(0,"O")
		e1:SetLabel(code)
		e1:SetTarget(s.ootar211)
		e1:SetReset(RESET_PHASE+PHASE_END)
		Duel.RegisterEffect(e1,tp)
		local e2=MakeEff(c,"FC")
		e2:SetCode(EVENT_CHAIN_SOLVING)
		e2:SetLabel(code)
		e2:SetCondition(s.oocon212)
		e2:SetOperation(s.ooop212)
		e2:SetReset(RESET_PHASE+PHASE_END)
		Duel.RegisterEffect(e2,tp)
		local e3=e1:Clone()
