...
function s.tfil2(c)
	return c:IsCode(18454261) and c:IsAbleToHand()
end
...
function s.cfil3(c,tp)
	return c:IsSetCard("골도이드") and c:IsType(TYPE_MONSTER) and not c:IsPublic() and c:IsAbleToDeck()
		and Duel.IETarget(s.tfil3,tp,"G",0,1,nli,c:GetCode())
end
function s.tfil3(c,code)
	return c:IsSetCard("골도이드") and c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and c:ListsCode(code)
end
function s.tar3(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then
		return Duel.IEMCard(s.cfil3,tp,"H",0,1,nil,tp)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONFIRM)
	local g=Duel.SMCard(tp,s.cfil3,tp,"H",0,1,1,nil,tp)
	local tc=g:GetFirst()
	...
	Duel.ConfirmCards(1-tp,g)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local sg=Duel.STarget(tp,s.tfil3,tp,"G",0,1,1,nil,code)
...
function s.op3(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and Duel.SendtoHand(tc,nil,REASON_EFFECT)>0 and tc:IsLoc("H")
		and sc:IsRelateToEffect(e) then
