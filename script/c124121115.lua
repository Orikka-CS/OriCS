...
function s.tfil1(c,e,tp)
	return c:IsSetCard(0xfa7) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and not c:IsCode(id)
end
...
function s.op1(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.tfil1),tp,LOCATION_DECK+LOCATION_GRAVE,0,1,1,
		nil,e,tp)
	if #g>0 and Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)>0 and c:IsAbleToHand()
...
function s.op3(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif op==2 then
		...
		local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.tfil3),tp,0,LOCATION_GRAVE+LOCATION_REMOVED
			,1,1,nil)
		if #g>0 then
			Duel.HintSelection(g)
