...
function s.tg2ffilter(c,cd)
	return c:IsSetCard(0xf2f) and c:IsAbleToGrave() and not c:IsCode(cd)
end

function s.tg2filter(c,e,tp)
	return Duel.IsExistingMatchingCard(s.tg2ffilter,tp,LOCATION_DECK,0,1,nil,c:GetCode()) and c:IsCanBeEffectTarget(e) and c:IsSetCard(0xf2f) and not c:IsCode(id) and c:IsFaceup()
end
