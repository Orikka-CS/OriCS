...
function s.cfil11(c,e,tp,chain,ec)
	if c:IsSetCard(SET_NUMBER) or not c:IsType(TYPE_XYZ)
		or not (c:IsRankAbove(5) and c:IsRankBelow(8))
...
function s.tfil22(c,e)
	return c:IsSetCard(0xc05) and not c:IsCode(id) and not c:IsImmuneToEffect(e)
end
function s.tar2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then
		return Duel.IsExistingTarget(s.tfil21,tp,LOCATION_MZONE,0,1,nil)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,s.tfil21,tp,LOCATION_MZONE,0,1,1,nil)
	local b=Duel.IsExistingMatchingCard(s.tfil22,tp,LOCATION_DECK,0,1,nil,e)
	local op=Duel.SelectEffect(tp,{b,aux.Stringid(id,1)},{true,aux.Stringid(id,2)})
	e:SetLabel(op)
end
function s.op2(e,tp,eg,ep,ev,re,r,rp)
	...
	local op=e:GetLabel()
	if op==1 and tc:IsFaceup() and tc:IsType(TYPE_XYZ) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_XMATERIAL)
		local g=Duel.SelectMatchingCard(tp,s.tfil22,tp,LOCATION_DECK,0,1,1,nil,e)
		local oc=g:GetFirst()
		if oc then
