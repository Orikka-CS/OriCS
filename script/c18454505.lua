--“마녀사냥 당한 마녀가, 지금 이곳에 돌아왔어”
local s,id=GetID()
function s.initial_effect(c)
	local e1=MakeEff(c,"A")
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_REMOVE+CATEGORY_SEARCH+CATEGORY_DRAW)
	WriteEff(e1,1,"TO")
	c:RegisterEffect(e1)
end
...
function s.tar1(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	Duel.SPOI(0,CATEGORY_DRAW,nil,1,tp,0)
end
function s.ofil1(c,e,tp)
	return c:IsSetCard("춘향") and ((Duel.GetLocCount(tp,"M")>0 and c:IsCanBeSpecialSummoned(e,0,tp,false,false))
		or c:IsAbleToHand()) and c:IsType(TYPE_MONSTER)
end
function s.op1(e,tp,ep,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) then
		...
		if chk>0 then
			local g=Duel.GMGroup(s.ofil1,tp,"D",0,nil,e,tp)
			if #g>0 and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then
				Duel.BreakEffect()
				Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
				local sg=g:Select(tp,1,1,nil)
				local sc=sg:GetFirst()
				local b3=sc:IsAbleToHand()
				local b4=Duel.GetLocCount(tp,"M")>0 and sc:IsCanBeSpecialSummoned(e,0,tp,false,false)
				local op=Duel.SelectEffect(tp,
					{b3,aux.Stringid(id,4)},
					{b4,aux.Stringid(id,0)})
				local schk=0
				if op==1 then
					schk=Duel.SendtoHand(sc,nil,REASON_EFFECT)
					if schk>0 then
						Duel.ConfirmCards(1-tp,sc)
					end
				elseif op==2 then
					schk=Duel.SpecialSummon(sc,0,tp,tp,false,false,POS_FACEUP)
				end
				if schk>0 and Duel.IsPlayerCanDraw(tp,1) and Duel.SelectYesNo(tp,aux.Stringid(id,3)) then
					Duel.BreakEffect()
