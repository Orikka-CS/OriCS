...
function s.oop13(e,tp,eg,ep,ev,re,r,rp)
	...
	local changeop=false
	for i=1,cc do
		local ce,cg=Duel.GetChainInfo(i,CHAININFO_TRIGGERING_EFFECT,CHAININFO_TARGET_CARDS)
		if cg and cg:IsContains(rc) and not rc:IsRelateToEffect(ce) then
			changeop=true
		end
	end
	if changeop then
		local g=Group.CreateGroup()
		Duel.ChangeTargetCard(cc,g)
		Duel.ChangeChainOperation(cc,function()
		end)
	end
end
...
function s.tar2(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	local g=Duel.GMGroup(s.tfil2,tp,"O","O",c)
	if chk==0 then
		return #g>0
	end
...
function s.op2(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SMCard(tp,s.tfil2,tp,"O","O",1,1,nil)
	if #g>0 then
