...
function s.tdfilter(c,e)
	return ((c:IsSetCard(0x12da) and c:IsMonster()) or (c:IsSetCard(0x2da) and c:IsType(TYPE_TRAP)))
...
function s.spconfilter(c,tp)
	return c:IsSetCard(0x2da) and c:IsControler(tp) and c:IsFaceup()
end
function s.spcon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(s.spconfilter,1,nil,tp)
end
function s.spfilter(c)
	return c:IsSetCard(0x12da) and c:IsLinkSummonable()
end
