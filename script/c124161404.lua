...
function s.cst1filter(c)
	return c:IsSetCard(0xf3a) and c:IsType(TYPE_FUSION) and not c:IsPublic()
end
...
function s.op1(e,tp,eg,ep,ev,re,r,rp)
	...
	if tg and tg:IsFaceup() then
		...
		tg:CopyEffect(code,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END)
		Duel.BreakEffect() 
...
function s.con2filter(c,tp)
	return c:IsControler(tp) and c:IsSetCard(0xf3a)
end
