...
function s.tg1filter(c)
	return c:IsSpellTrap() and c:IsSetCard(0xf24) and c:IsSSetable()
end
...
--effect 2
function s.con2filter(c,e,tp,re,r,rp)
	return rp==tp and c:IsPreviousLocation(LOCATION_MZONE) and c:IsLocation(LOCATION_GRAVE) and c:IsPreviousControler(1-tp) and c:IsReason(REASON_EFFECT) and c:IsCanBeEffectTarget(e) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end

function s.con2(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(s.con2filter,1,nil,e,tp,re,r,rp)
end

function s.tg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and s.con2filter(chkc,e,tp,re,r,rp) end
	local g=eg:Filter(s.con2filter,nil,e,tp,re,r,rp)  
