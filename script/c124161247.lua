...
function s.con2filter(c,tp)
	return c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and c:IsSetCard(0xf30) and c:IsType(TYPE_FUSION) and c:IsSummonLocation(LOCATION_EXTRA)
end
...
function s.op2filter(c,e,tp)
	return c:IsSetCard(0xf30) and c:IsMonster() and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
