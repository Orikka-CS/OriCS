...
function s.op2filter(c,e,tp)
	return c:IsSetCard(0xf30) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.op2(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) then
		local b=Duel.SelectOption(tp,aux.Stringid(id,0),aux.Stringid(id,1))
		Duel.DisableShuffleCheck()
		if Duel.SendtoDeck(c,nil,b==0 and SEQ_DECKTOP or SEQ_DECKBOTTOM,REASON_EFFECT)>0 and c:IsLocation(LOCATION_DECK) and Duel.Draw(tp,1,REASON_EFFECT)>0 then
