	...
	if chk==0 then
		return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
			and Duel.IsPlayerCanSpecialSummonMonster(tp,id,0xc02,
				TYPE_MONSTER|TYPE_NORMAL|TYPE_TUNER,0,0,3,RACE_FAIRY,ATTRIBUTE_WATER)
	end
...
function s.op2(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e)
		and Duel.IsPlayerCanSpecialSummonMonster(tp,id,0xc02,
			TYPE_MONSTER|TYPE_NORMAL|TYPE_TUNER,0,0,3,RACE_FAIRY,ATTRIBUTE_WATER) then
...
function s.tfil3(c)
	return c:IsSetCard(0xc02) and c:IsAbleToHand() and c:IsType(TYPE_SPELL+TYPE_TRAP) and not c:IsCode(id)
end
