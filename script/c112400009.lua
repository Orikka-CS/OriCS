--ポイズンリーフゼリッピ
local s,id=c112400009,112400009
...
s.listed_names={112400002,112400007,112400010}
s.card_code_list={[112400002]=true,[112400007]=true,[112400010]=true}
...
function s.sstfilter(c,sc,sumtype,tp)
	return s.IsSynchroCode(c,112400007,sc,sumtype,tp) or c:IsHasEffect(112400008)
end
--me1(def up)
...
function s.me2filter2(c,tp)
	return c:IsType(TYPE_MONSTER) and c:IsAbleToRemove()
		and Duel.IsExistingTarget(s.me2filter,tp,0,LOCATION_ONFIELD,1,c)
end
function s.me2tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsOnField() and chkc:IsAbleToRemove() end
	if chk==0 then return Duel.IsExistingTarget(s.me2filter2,tp,0,LOCATION_ONFIELD,1,nil,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g1=Duel.SelectTarget(tp,s.me2filter2,tp,0,LOCATION_ONFIELD,1,1,nil,tp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g2=Duel.SelectTarget(tp,s.me2filter,tp,0,LOCATION_ONFIELD,1,1,g1:GetFirst())
	g1:Merge(g2)
